...
angular.module('cdi.services', ['restangular'])
    ...
    .factory('executionsResource', ['$q','Restangular', function($q, Restangular) {
        ...
        return {
             ...
             * @returns {Promise}
             */
            getPagedJobExecutions: function(jobID, limitTo, recordOffset, orderBy, sort) {
                var pageConfig = {
            ...
            , buildCronString : function(scheduleTemplate) {
                ...
                try {
                    switch (_mode) {
                        ...
                        case _modes.RECURRING: {
                            cronString = scheduleTemplate.minute.value + " " + scheduleTemplate.hour.value + " ";
                            if (scheduleTemplate.customerOption == 'daily') {
                                cronString += (scheduleTemplate.daily.weekdaysOnly ? "?" : "1/" + scheduleTemplate.daily.repeatDays) + " * " +
                                (scheduleTemplate.daily.weekdaysOnly ? "MON-FRI" : "?") + " *";
                            }
                            else if (scheduleTemplate.customerOption == 'weekly') {
                                var days = "";
                                for (var i = 0; i < scheduleTemplate.weekly.daysOfWeek.length; i++) {
                                    if (days == "")
                                        days += scheduleTemplate.weekly.daysOfWeek[i].label;
                                    else
                                        days += "," + scheduleTemplate.weekly.daysOfWeek[i].label;
                                }
                                cronString += "? * " + days + " *";
                            ...
                            else if (scheduleTemplate.customerOption == 'monthly') {


                                if (scheduleTemplate.monthly.repeatOn != "Day") {
                                    var weeks = ["First", "Second", "Third", "Fourth"];
                                    var week = weeks.indexOf(scheduleTemplate.monthly.repeatOn) + 1;



                                    if (scheduleTemplate.monthly.repeatDay == "Weekday" || scheduleTemplate.monthly.repeatDay == "Day") {
                                        if (scheduleTemplate.monthly.repeatOn == "Last") {
                                            cronString += "L";
                                        }
                                        else {
                                            cronString += (((week - 1) * 7) + 1)
                                        }
                                        cronString +=  (scheduleTemplate.monthly.repeatDay == "Day"?  "" : "W") +
                                        " 1/" + scheduleTemplate.monthly.repeatMonths + " ?";

                                    }
                                    else {
                                        cronString += "?" + " 1/" + scheduleTemplate.monthly.repeatMonths + " ";

                                        if (scheduleTemplate.monthly.repeatOn == "Last") {
                                            cronString += scheduleTemplate.monthly.repeatDay + "L";
                                        }
                                        else {
                                            cronString += scheduleTemplate.monthly.repeatDay + "#" + week;
                                        }
                                    }
                                    ...
                                    cronString += scheduleTemplate.monthly.repeatDay +
                                    " 1/" + scheduleTemplate.monthly.repeatMonths + " ?";
                                }
